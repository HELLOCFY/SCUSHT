...
Page({
  ...
  onLoad: function (options) {
    wx.request({
      url: "https://example.com/query/mysave/", //仅为示例，并非真实的接口地址
      method: 'POST',
      data: {
      ...
      success: res => {
        var i = 0;
        for (i in res.data) {
          res.data[i].post_time = res.data[i].post_time.substring(0, 10) + " " + res.data[i].post_time.substring(res.data[i].post_time.length - 8, res.data[i].post_time.length)
        }
        this.setData({
